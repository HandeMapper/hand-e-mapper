...
public class HaarClassifierGesture extends AbstractGesture {
	...
	private static final Logger logger = LogManager.getLogger(HaarClassifierGesture.class);
	...
	public static final CascadeClassifier loadClassifier(String cPath) {
		try {
			...
			if (f != null && f.canRead())
				...
				logger.error("Unable to read the resource: " + cPath);
		}
		catch (NullPointerException npe) {
			logger.error("Could not locate resource: [classifier] " + cPath);
			logger.error(" -- Current resource path: " + HaarClassifierGesture.class.getName());
		}
		catch (Exception ex) {
			logger.error("Unable to load resource: [classifier] " + cPath);
		}
		return null;
	}
